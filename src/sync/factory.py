...
from .bidirectional import BidirectionalSyncWorkflow
...
def build_sync_workflow(settings: Settings):
    settings.validate()
    ...
    dataverse = DataverseClient(auth, api_version=settings.dynamics_api_version)
    ...
    return BidirectionalSyncWorkflow(
        ...
        reverse=ReverseContactSync(
            dynamics=dataverse,
            hubspot=hubspot,
            integration_user_id=settings.dynamics_integration_user_id,
        ),
        checkpoint=CheckpointStore(
            bucket_name=settings.checkpoint_bucket,
            blob_name=settings.checkpoint_blob,
        ),
    )
