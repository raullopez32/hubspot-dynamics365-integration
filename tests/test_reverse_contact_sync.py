...
def test_read_changed_excludes_integration_user():
    ...
    contacts = sync.read_changed("2026-08-20T11:00:00Z")
    ...
    dynamics.request.assert_called_once_with(
        ...
        params={
            ...
            "$filter": "modifiedon gt 2026-08-20T11:00:00Z and _modifiedby_value ne integration-user",
...
def test_existing_hubspot_contact_is_updated():
    hubspot = mock.Mock()
    hubspot.search_by_email.return_value = [{"id": "hs-1"}]
...
def test_missing_hubspot_contact_is_not_created():
    ...
    assert result == {"source_id": "contact-1", "status": "missing"}
...
def test_contact_without_email_is_skipped():
    ...
    assert result == {"source_id": "contact-1", "status": "skipped"}
